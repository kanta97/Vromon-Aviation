...
const roleMenuController = require('express').Router()

const bodyParser = require('body-parser')

roleMenuController.use(bodyParser.urlencoded({extended: true}))
roleMenuController.use(bodyParser.json())
...
roleMenuController.use(function (request, response, next) {
...
roleMenuController.route('/search').get(function (request, response){
    try {
        ...
        Auth.checkToken(request.headers.auth_token).then((result) => {
            ...
            if (!result) {
                ...
                let condition = 'SELECT menus.menu AS menu_name,menus.display_menu AS menu_display_name, menus.id AS menu_id, roles.id AS role_id, roles.role_name, roles.display_role_name FROM role_menus INNER JOIN roles ON roles.id = role_menus.role INNER JOIN menus ON menus.id = role_menus.menu '
                let count_query = 'SELECT COUNT(role_menus.id) AS count FROM role_menus INNER JOIN roles ON roles.id = role_menus.role INNER JOIN menus ON menus.id = role_menus.menu '
                let replace = []
                let replace_counter = []
                let flag = false
                ...
                if(id !== '' && id !== undefined && id !== null){
                    ...
                    condition += ' role_menus.id = ? ';
                    count_query += ' role_menus.id = ? ';
                    replace.push(id);
                ...
                if(role !== '' && role !== undefined && role !== null){
                    ...
                    condition += ' role_menus.role = ? ';
                    count_query += ' role_menus.role = ? ';
                    replace.push(role);
                ...
                if(menu !== '' && menu !== undefined && menu !== null){
                    ...
                    condition += ' role_menus.menu = ? ';
                    count_query += ' role_menus.menu = ? ';
                    replace.push(menu);
                    replace_counter.push(menu);
                }
                ...
                if(deletedAt !== '' && deletedAt !== undefined && deletedAt !== null){
                    ...
                    condition += ' role_menus.deletedAt = > ';
                    count_query += ' role_menus.deletedAt = > ';
                ...
                if(createdBy !== '' && createdBy !== undefined && createdBy !== null){
                    ...
                    condition += ' role_menus.createdBy = ? ';
                    count_query += ' role_menus.createdBy = ? ';
                ...
                if(updatedBy !== '' && updatedBy !== undefined && updatedBy !== null){
                    ...
                    condition += ' role_menus.updatedBy = ? ';
                    count_query += ' role_menus.updatedBy = ? ';
                ...
                if(createdAt !== '' && createdAt !== undefined && createdAt !== null){
                    ...
                    condition += ' role_menus.createdAt > ? ';
                    count_query += ' role_menus.createdAt > ? ';
                ...
                if(updatedAt !== '' && updatedAt !== undefined && updatedAt !== null){
                    ...
                    condition += ' role_menus.updatedAt > ? ';
                    count_query += ' role_menus.updatedAt > ? ';
                    replace.push(updatedAt);
                    replace_counter.push(updatedAt);
                }
                condition += ' order by role_menus.id desc ';
...
roleMenuController.route('/:id').get(function (request, response){
    try {
        let Auth = new ApiAccess(request)
        Auth.checkToken(request.headers.auth_token).then((result) => {
            //console.log(result)
            if (!result) {
                ...
                const permission_sql = `SELECT menus.menu AS menu_name,menus.display_menu AS menu_display_name,
                    menus.id AS menu_id, roles.id AS role_id, roles.role_name, roles.display_role_name
                    FROM role_menus
                    INNER JOIN roles ON roles.id = role_menus.role
                    INNER JOIN menus ON menus.id = role_menus.menu where role_menus.deletedAt IS NULL AND role_menus.id = ? `;
                db.sequelize.query(
                    permission_sql,
                    {
...
roleMenuController.route('/').post(function (request, response){
    try {
        let Auth = new ApiAccess(request)
        Auth.checkToken(request.headers.auth_token).then((result) => {
            if (!result) {
                ...
                try {
                    ...
                    db.role_menus.create(new_role_menus).then(function (role_menus) {
                        try {
                            if (role_menus) {
                                response.json({success: true, message: 'Role applied successfully on menu'})
                            } else {
...
roleMenuController.route('/:id').patch(function (request, response){
    try {
        let Auth = new ApiAccess(request)
        Auth.checkTokenWithUser(request.headers.auth_token, request.body.updated_by).then((result) => {
            if (!result) {
                ...
                try {
                    db.role_menus.find({where: {id: request.params.id, deletedAt:null}})
...
roleMenuController.route('/:id').delete(function (request, response){
    try {
        let Auth = new ApiAccess(request)
