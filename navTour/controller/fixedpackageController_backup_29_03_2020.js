...
exports.search_package_by_packageid = function (req, res) {
  Fixedpackage.getPackageDetailsById(req.params.packageid, function (err, response) {
    if (err) {
      var resp = new Resp({"package":req.body,"message":respMsg.get(responseMessageKey.package.search.detail_fail)}, "400 Bad Request");
      res.status(400).json(resp);
    }
    else {
  ...
  Fixedpackage.getPackageByDestination(req, function(err, fixedpackage,records) {
    if (err) {
      ...
      if(records>=1){
          ...
          var resp = new Response({ "packages" :null,"message": respMsg.get(responseMessageKey.package.search.fail)}, "200 OK",records);
          res.status(200).json(resp);
      }
