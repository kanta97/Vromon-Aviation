...
class FailedCountry extends Component {
  constructor(props){
    super(props)
    this.state = {
      hotelData: [],
  ...
  async componentDidMount(){
      let data = await actionTypes.loadCountryData();
  ...
  refreshActivity =(event)=>{
      ...
      actionTypes.refreshActivityCountry()
          .then((response) => {
  ...
  render() {
    ...
    if(this.state.hotelData.length > 0){
        ...
        tableBody = this.state.hotelData.map((value, index) => {
            k = index+1;
            totalCount++;
            return <tr key={index}><td>{k}</td><td>{value.CountryId}</td></tr>
        })
      }
     return (
      <Fragment>
