...
class CustomHotel extends Component {
  constructor(props) {
    super(props);
    this.state = {
      ...
      .then((res) => {
        ...
        if(localStorage.getItem('userRole')==6){
            ...
            for(let i=0; i<res.rows.length;i++){
                if(res.rows[i].country=='Bangladesh' && res.rows[i].id>143){
        ...
        if (response.data.status === "200 OK") {
          if (response.data.body.length > 0) {
            this.setState({
              ...
              infoModalTitle:
                "Hotel view of ID " + response.data.body[0].id,
            });
          } else {
            this.setState({
              danger: true,
