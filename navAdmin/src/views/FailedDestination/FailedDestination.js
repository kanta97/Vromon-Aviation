...
class FailedDestination extends Component {
  constructor(props){
    super(props)
    this.state = {
      hotelData: [],
  ...
  async componentDidMount(){
      let data = await actionTypes.loadDestinationData();
  ...
  refreshActivity =(event)=>{
      ...
      actionTypes.refreshActivityDestination()
          .then((response) => {
  ...
  render() {
    ...
    if(this.state.hotelData.length > 0){
        ...
        tableBody = this.state.hotelData.map((value, index) => {
            k = index+1;
            totalCount++;
            return <tr key={index}><td>{k}</td><td>{value.DestinationId}</td></tr>
        })
      }
     return (
      <Fragment>
                ...
                <div className={'row'}>
                    ...
                    <div className={'col-6'}>
                        <div className='text-right add_btn'>
                            <Button color="primary" onClick={this.refreshActivity}><b>Retry for data</b></Button>
                        </div>
                    </div>
                </div>
...
export default FailedDestination;
