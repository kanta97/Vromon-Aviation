...
export default function Roundway(props) {
  const {
    ...
    cabin,
    handleCabin,
    handleSwapper,showDatePicker, setShowDatePicker,adultList,infantList,isOffcanvasOpenCal, setIsOffcanvasOpenCal,isOffcanvasOpend, setIsOffcanvasOpen,isOffcanvasOpenTo, setIsOffcanvasOpenTo,showDropdownr, setShowDropdownr
...
// Control visibility of the date picker
  ...
  //     setShowDropdownr(false); // Close dropdown if clicked outside
  //   }
  // };
  ...
  const customStyles = {
    option: (provided, state) => ({
      ...
      backgroundColor: state.data.isFirst
        ? "#b0e7e5" // Always red for the first option
        : state.isFocused
...
const uniqueOptions = (options) => {
      ...
      >
        ...
        {showDatePicker && (
          <div
            ...
            className="datepicker-wrapper date_sm"
            style={{
  ...
  <div className='text-center'>
    <h6 className='mt-1 mb-0' id="offcanvasTopLabel"> {startDate ? moment(startDate).format("MMM DD, YYYY") : "N/A"} - {endDate
                  ? moment(endDate).format("MMM DD, YYYY")
                  : "Select Date"}</h6>
